...
require(['api'], function (api) {
    ...
    function LandingZone(zone) {
        ...
        self.index = ko.observable(zone.index).extend({
            numeric: 0
        });
        self.minArmies = ko.observable(zone.min).extend({
            numeric: 0
        });
        self.maxArmies = ko.observable(zone.max).extend({
            numeric: 0
        });
    ...
    function SystemEditorViewModel() {
        ...
        self.screenOffset = function () {
            /* doesn't do anything anymore... it will resize the c++ viewport latter. */
        ...
        self.planets = ko.observableArray([]).extend({
            local: 'planets'
        });
        self.systems = ko.observableArray([]).extend({
            db: {
                local_name: 'systems',
                db_name: 'misc'
            }
        ...
        self.buildBarHoverRadiusWidthString = ko.computed(function () {
            if (!jQuery.isEmptyObject(self.buildBarHoverPlanet())) return String(self.buildBarHoverPlanet().planet.radius / 20) + 'px';
        });
        self.buildBarHoverHeightWidthString = ko.computed(function () {
            if (!jQuery.isEmptyObject(self.buildBarHoverPlanet())) return String(self.buildBarHoverPlanet().planet.heightRange) + 'px';
        });
        self.buildBarHoverWaterWidthString = ko.computed(function () {
            if (!jQuery.isEmptyObject(self.buildBarHoverPlanet())) return String(self.buildBarHoverPlanet().planet.waterHeight) + 'px';
        });
        self.buildBarHoverTemperatureWidthString = ko.computed(function () {
            if (!jQuery.isEmptyObject(self.buildBarHoverPlanet())) return String(self.buildBarHoverPlanet().planet.temperature) + 'px';
        });
        self.defaultTemplates = ko.observableArray(templates);
        self.recentTemplates = ko.observableArray([]).extend({
            local: 'recentPlanets'
        });
        self.buildbarPageSize = ko.computed(function () {
            return Math.floor(($('#page').width() - 140) / 65);
        })
        ...
        self.UpdateRecentTemplates = function (planet) {
            ...
            var i = _.findIndex(self.recentTemplates(), {
                name: p.name
            });
            if (i >= 0) {
                if (i > 0) {
        ...
        self.validSelectedPlanet = ko.computed(function () {
            return self.selectedPlanetIndex() >= 0;
        })

        self.paused = ko.observable(true);
        self.simulating = ko.computed(function () {
            return !self.paused();
        });
        ...
        self.loadedPlanet = ko.observable().extend({
            session: 'loaded_planet'
        });
        ...
        self.radius = ko.observable(700).extend({
            slider: 'radius'
        });
        self.heightRange = ko.observable(50).extend({
            slider: 'height_range'
        });
        self.biomeScale = ko.observable(50).extend({
            slider: 'biome_scale'
        });
        self.waterHeight = ko.observable(50).extend({
            slider: 'water_height'
        });
        self.waterDepth = ko.observable(50).extend({
            slider: 'water_depth'
        });
        self.temperature = ko.observable(50).extend({
            slider: 'temperature'
        });
        self.metalDensity = ko.observable(50).extend({
            slider: 'metal_density'
        });
        self.metalClusters = ko.observable(50).extend({
            slider: 'metal_clusters'
        });
        self.radius = ko.observable(700).extend({
            slider: 'radius'
        });
        ...
        self.numArmies = ko.observable(2).extend({
            slider: 'num_armies'
        });
        self.numArmies.subscribe(function (value) {
            ...
            engine.call('execute', 'update_planet_num_armies', JSON.stringify({
                num_armies: self.numArmies()
            }));
        });

        self.landingZonesPerArmy = ko.observable(self.valueRanges.landingZonesPerArmy.min).extend({
            slider: 'landing_zones_per_army'
        });
        ...
        self.landingZoneSize = ko.observable(self.valueRanges.landingZoneSize.min).extend({
            slider: 'landing_zones_size'
        });
        ...
        self.mass = ko.observable(2500).extend({
            slider: 'mass'
        });
        ...
        self.not_starting_planet = ko.computed(function () {
            return !self.starting_planet()
        });
        self.displayOrbit = ko.observable();
        self.sandboxPhysics = ko.observable();

        self.biomeError = ko.computed(function () {
            return ''
        });
        self.waterHeightError = ko.computed(function () {
            return self.waterHeight() > 60 ? '!' : ''
        });
        ...
        self.disableNumArmies = ko.computed(function () {
            return !self.starting_planet() || self.planetUnderConstruction()
        });
        ...
        self.currentAsteroidLimit = ko.observable(DEFAULT_ASTEROID_LIMIT).extend({
            numeric: 0
        });
        self.startingAsteroidCount = ko.observable(DEFAULT_STARTING_ASTEROID_COUNT).extend({
            numeric: 0
        });
        self.minAsteroidSpawnDelay = ko.observable(DEFAULT_MIN_ASTEROID_SPAWN_DELAY).extend({
            numeric: 0,
            positive: true
        });
        self.asteroidSpawnDelayVariance = ko.observable(DEFAULT_ASTEROID_SPAWN_VARIANCE).extend({
            numeric: 0,
            positive: true
        });
        ...
        self.systemAsJson = ko.computed(function () {
            return {
                ...
                planets: _.map(self.system().planets, function (planet) {
                    return _.cloneDeep(planet);
                })
            }
        });
        ...
        self.bindAdvancedSlider = function (name, id, sliderStopFunction) {
            ...
            result.on('slideStop', function (event) {
                self[name](event.value);
                if (sliderStopFunction) sliderStopFunction();
            });
        ...
        self.biomeJsonsObs = ko.observable().extend({
            session: 'biome_jsons'
        });
        ...
        self.handleHideControls = function () {
            ...
            engine.call('execute', 'set_planet_offset', JSON.stringify({
                x_offset: 0,
                y_offset: 0
            }));
        }
        ...
        self.setStartingPlanet = function (index) {
            engine.call('execute', 'set_start_planet', JSON.stringify({
                index: index
            }));
        }
        ...
        self.planetSummary = ko.computed(function () {
            ...
            var planets = [{
                name: 'Sun',
                imageSmall: 'coui://ui/main/shared/img/img_system_generic_icon.png',
                isSelected: false,
                index: 'sun'
            }];
            ...
            _.forEach(self.system().planets, function (planet, index) {
                ...
                planets.push({
                    name: planet.name,
                    imageSmall: 'coui://ui/main/shared/img/' + biome + '.png',
                    index: index,
        ...
        self.setup = function () {
            ...
            $('#div_planet_landing_zones_list').hover(function () {
                engine.call("game.allowKeyboard", false);
            }, function () {
                engine.call("game.allowKeyboard", true);
            });
            $('#div_csg_json').hover(function () {
                engine.call("game.allowKeyboard", false);
            }, function () {
                engine.call("game.allowKeyboard", true);
            });
